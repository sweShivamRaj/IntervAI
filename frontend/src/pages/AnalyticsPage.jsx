...
export default function AnalyticsPage() {
  ...
  if (error) return <div className="mx-auto max-w-lg space-y-4"><Alert>{error}</Alert><div className="text-center"><button type="button" className="btn-secondary transition-all active:scale-[0.97]" onClick={loadAnalytics}>Try again</button></div></div>;
  ...
  return (
    <div className="space-y-8">
      <FadeIn>
        <PageHeader eyebrow="Measure what matters" title="Performance analytics" description="Use completed sessions to understand your progress and choose your next focus area." actions={<Link to="/history" className="btn-secondary transition-all active:scale-[0.97]"><Icon name="history" size={16} /> View history</Link>} />
      </FadeIn>
      
      <StaggerList className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StaggerItem><Metric icon="file" label="Total interviews" value={stats.totalInterviews ?? 0} /></StaggerItem>
        <StaggerItem><Metric icon="target" label="Average score" value={stats.completedInterviews ? `${Math.round(stats.averageScore)}%` : '—'} /></StaggerItem>
        <StaggerItem><Metric icon="chart" label="Best score" value={stats.completedInterviews ? `${Math.round(stats.bestScore)}%` : '—'} /></StaggerItem>
        <StaggerItem><Metric icon="spark" label="Strongest skill" value={stats.strongestSkill || '—'} /></StaggerItem>
        <StaggerItem><Metric icon="arrowUp" label="Focus next" value={stats.weakestSkill || '—'} tone="amber" /></StaggerItem>
        <StaggerItem><Metric icon="history" label="Improvement trend" value={trendLabel(stats.improvementTrend)} tone={stats.improvementTrend?.direction === 'down' ? 'amber' : 'teal'} /></StaggerItem>
      </StaggerList>
      ...
      {stats.completedInterviews === 0 && (
        <FadeIn>
          <EmptyState icon="chart" title="Analytics unlock after completion" description="Complete an interview to see historical performance, skill averages, and improvement trends." action={<Link to="/interview/setup" className="btn-primary transition-all active:scale-[0.97]">Start an interview <Icon name="arrow" size={16} /></Link>} />
        </FadeIn>
      )}
    </div>
  );
...
function ChartCard({ title, subtitle, empty, emptyTitle, emptyText, children }) {
  return <section className="card-panel h-full"><div className="mb-5"><h2 className="section-title">{title}</h2><p className="mt-1 text-sm text-ink-700">{subtitle}</p></div>{empty ? <EmptyState icon="chart" title={emptyTitle} description={emptyText} /> : <div className="h-72 w-full">{children}</div>}</section>;
}
...
function Metric({ icon, label, value, tone = 'navy' }) {
  ...
  return <div className="card-panel h-full flex items-start gap-3"><span className={`grid size-10 place-items-center rounded-xl ${iconClass}`}><Icon name={icon} size={18} /></span><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.1em] text-ink-700">{label}</p><p className="mt-2 truncate font-display text-2xl font-semibold text-ink-900">{value}</p></div></div>;
}
