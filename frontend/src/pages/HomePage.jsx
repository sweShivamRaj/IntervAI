    ...
    <div className="space-y-20 pb-8">
        ...
        <div className="relative grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to={user ? '/interview/setup' : '/register'} className="btn-primary bg-accent-soft text-ink-900 hover:bg-white">
                {user ? 'Start an interview' : 'Create free account'} <Icon name="arrow" size={17} />
              </Link>
              <Link to={user ? '/dashboard' : '/login'} className="inline-flex items-center justify-center rounded-xl border border-white/20 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/10">
          ...
          <div className="relative lg:pl-8">
            <div className="rounded-3xl border border-white/15 bg-white/[0.08] p-4 shadow-2xl backdrop-blur sm:p-5">
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {steps.map((step) => <div key={step.number} className="card-panel group relative overflow-hidden transition-transform hover:-translate-y-0.5"><span className="font-display text-5xl font-semibold text-accent/20">{step.number}</span><h3 className="mt-5 text-lg font-bold text-ink-900">{step.title}</h3><p className="mt-2 text-sm leading-6 text-ink-700">{step.text}</p></div>)}
        </div>
      </section>
      ...
      <section className="grid items-center gap-8 rounded-3xl border border-accent/20 bg-accent-soft/45 p-7 sm:p-10 lg:grid-cols-[1fr_auto]">
        ...
        <Link to={user ? '/analytics' : '/register'} className="btn-secondary whitespace-nowrap">Explore analytics <Icon name="arrow" size={16} /></Link>
      </section>
