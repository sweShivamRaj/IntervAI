  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        actions={<Link to="/interview/setup" className="btn-primary"><Icon name="plus" size={17} /> Start new interview</Link>}
      />
...
function DashboardSkeleton() {
  return <div className="space-y-8" role="status"><div><Skeleton className="h-3 w-32" /><Skeleton className="mt-4 h-10 w-80 max-w-full" /><Skeleton className="mt-3 h-5 w-96 max-w-full" /></div><Skeleton className="h-36 w-full rounded-3xl" /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[1, 2, 3, 4].map((item) => <Skeleton key={item} className="h-32" />)}</div><div className="grid gap-6 lg:grid-cols-2"><Skeleton className="h-96" /><Skeleton className="h-96" /></div></div>;
}

function ErrorState({ message, onRetry }) {
  return <div className="mx-auto max-w-lg space-y-4"><Alert>{message}</Alert><div className="text-center"><button type="button" className="btn-secondary" onClick={onRetry}>Try again</button></div></div>;
}
