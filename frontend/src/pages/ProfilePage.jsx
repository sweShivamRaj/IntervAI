      ...
      <div className="grid gap-6 lg:grid-cols-[0.72fr_1.28fr]">
        ...
        <section className="card-panel">
          ...
          <form onSubmit={onSubmit} className="space-y-5" noValidate>
            <div className="grid gap-5 sm:grid-cols-2"><Field label="Full name" id="profile-name" error={fieldErrors.name}><input id="profile-name" className="input-field" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} aria-invalid={Boolean(fieldErrors.name)} aria-describedby={fieldErrors.name ? 'profile-name-error' : undefined} /></Field><Field label="Education" id="profile-education"><input id="profile-education" className="input-field" placeholder="e.g. BCA, MCA" value={form.education} onChange={(event) => setForm({ ...form, education: event.target.value })} /></Field></div>
            <Field label="Experience" id="profile-experience" hint="A short summary is enough."><textarea id="profile-experience" className="input-field min-h-32 resize-y" placeholder="Work, internships, projects, or areas of experience" value={form.experience} onChange={(event) => setForm({ ...form, experience: event.target.value })} /></Field>
            <Field label="Skills" id="profile-skills" hint="Separate multiple skills with commas."><input id="profile-skills" className="input-field" placeholder="JavaScript, React, Node.js" value={form.skills} onChange={(event) => setForm({ ...form, skills: event.target.value })} /></Field>
            <Field label="Resume or portfolio" id="profile-resume" hint="Add a link or a short summary."><textarea id="profile-resume" className="input-field min-h-24 resize-y" placeholder="https://… or a short professional summary" value={form.resume} onChange={(event) => setForm({ ...form, resume: event.target.value })} /></Field>
            {error && <Alert>{error}</Alert>}
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-ink-100 pt-5"><p className="text-xs text-ink-700"><Icon name="lock" size={13} className="mr-1 inline" /> Your information is only used inside your workspace.</p><button type="submit" className="btn-primary" disabled={busy}>{busy ? <><ButtonSpinner /> Saving…</> : <>Save changes <Icon name="check" size={16} /></>}</button></div>
          </form>
        </section>
      </div>
